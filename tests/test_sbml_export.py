...
def test_sbml_export():
    for model_id in BIOMODELS:
        ...
        assert len(tm0.get_concepts_name_map()) == len(
            tm1.get_concepts_name_map()
        )
        ...
        assert set(author.name for author in tm0_annots.authors) == set(
            author.name for author in tm1_annots.authors
        )
