...
def test_stockflow_assembley():
    ...
    # Don't text flow expressions as flow expressions are multipled by 1.0 and order of symbols between
    # input and output are not equivalent
    ...
    # test auxiliaries
    # Don't test for other fields as example input sir amr has multiplies symbol by 1.0 to demonstrate support for
    # expressions only. Non p_ prefix parameters have _aux suffix attached to id/name field in output amr
    ...
    for auxiliary in old_amr['model']['auxiliaries']:
        ...
        assert auxiliary['expression'][6:] == auxiliary_dict[auxiliary_id]['expression']
        assert expression_to_mathml(safe_parse_expr(auxiliary['expression'][6:])) == auxiliary_dict[auxiliary_id][
            'expression_mathml']
    ...
    # can't do same for links because no direct way to store links
